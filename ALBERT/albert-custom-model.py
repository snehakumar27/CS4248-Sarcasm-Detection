######################################################
## ALBERT Base Model                                ##
## 1. Adding Custom layers to ALBERT Transformers   ##
...
from transformers import AutoTokenizer, AlbertModel, PreTrainedTokenizerBase
...
class AlbertCustomisedModel(nn.Module):
    def __init__(self, num_fc_layers: int = 1, num_classes: int = 2):
        super().__init__()
        ...
        self.fc_layers = [
            (nn.Linear(in_features=4096, out_features=4096), nn.Dropout(p=0.25))
            for _ in range(self.num_fc_layers)
        ]
        self.linear_last = nn.Linear(in_features=4096, out_features=1)
        self.tanh = nn.Tanh()


    def forward(self, x: torch.Tensor) -> torch.Tensor:
        res = self.albert(x).pooler_output
        res = self.dropout_1(res)
        # res will have a size of (batch_size, sequence_length, hidden_size)
        for fc_layer in self.fc_layers:
            res = fc_layer[1](fc_layer[0](res))
        res = self.linear_last(res)
...
train_dev_data_series = pd.read_csv(TRAIN_DATA_FILE)["text"].iloc[0:100].tolist()
train_dev_label_series = pd.read_csv(TRAIN_LABEL_FILE)["label"].iloc[0:100].tolist()
test_data_series = pd.read_csv(TEST_DATA_FILE)["text"].iloc[0:100].tolist()
test_label_series = pd.read_csv(TEST_LABEL_FILE)["label"].iloc[0:100].tolist()
...
model = AlbertCustomisedModel(num_fc_layers=NUM_FC_LAYERS, num_classes=2)
model.to(DEVICE)
...
for epoch in range(NUM_EPOCHS):
    ...
    torch.save(checkpoint, f"albert_customised_fc_{2}_model_{epoch+1}.pt")
